...
public class MoveComponent : PoncherComponentBase
{
    ...
    private void FixedUpdate()
    {
        ...
        currentAngVel = m_poncherCharacter.GetRigidbody().angularVelocity;
    }
    ...
    public bool MoveTo(Vector3 destination, float stopDistance, bool ignoreY)
    {
        ...
        if (m_DistanceToTarget <= stopDistance)
            ...
            m_poncherCharacter.GetRigidbody().AddForce(relativePos * curAccel * Time.deltaTime, ForceMode.VelocityChange);

        return false; // Keep moving we havent arrive
    }
    public void ManageSpeed(float deceleration, float maxSpeed, bool ignoreY)
    {
        m_currentSpeed = m_poncherCharacter.GetRigidbody().velocity;
        if (ignoreY)
            m_currentSpeed.y = 0;

        if (m_currentSpeed.magnitude > 0)
        {
            m_poncherCharacter.GetRigidbody().AddForce((m_currentSpeed * -1) * deceleration * Time.deltaTime, ForceMode.VelocityChange);
            if (m_poncherCharacter.GetRigidbody().velocity.magnitude > maxSpeed)
                m_poncherCharacter.GetRigidbody().AddForce((m_currentSpeed * -1) * deceleration * Time.deltaTime, ForceMode.VelocityChange);
        }
    ...
    public void RotateVelocity(float turnSpeed, bool ignoreY)
    {
        ...
        if (ignoreY)
        {
            direction = new Vector3(m_poncherCharacter.GetRigidbody().velocity.x, 0f, m_poncherCharacter.GetRigidbody().velocity.z);
        }
        else
        {
            direction = m_poncherCharacter.GetRigidbody().velocity;
        }
        ...
        if (direction.magnitude > 0.1f)
        {
            ...
            m_poncherCharacter.GetRigidbody().MoveRotation(slerp);
        }
    }
